...
cardTables.forEach((cardTable, hall) => {
    ...
    days.forEach((d) => {
        ...
        div.innerHTML = `
            <div>${d.weekDay}</div>
            <div style="font-size:13px">${d.dateDay}</div>
        `;
...
const popupTitles = document.createElement("div");
popupTitles.id = "popup-titles";

const popupTitle = document.createElement("div");
popupTitle.id = "popup-title";

const popupSubtitle = document.createElement("div");
popupSubtitle.id = "popup-subtitle";
...
popupCard.append(popupTitles, popupButton);
popupTitles.append(popupTitle, popupSubtitle);
...
document.addEventListener("click", async (e) => {
    ...
    if (openCell) {
        ...
        popupTitle.innerHTML = `Дата: «<b>${openCell.dataset.day}</b>»`;
        popupSubtitle.innerHTML = `Время: «<b>${openCell.dataset.time}</b>»`;
